...
-- Toggle floating/tiling on windows
...
hl.bind(mainMod .. "+ Y", hl.dsp.exec_cmd(launchPrefix .. SPOTIFY))
...
hl.bind(mainMod .. " + Print", hl.dsp.exec_cmd(noctCall .. "screenshot-fullscreen all"))

-- Notifications
hl.bind(mainMod .. " + comma", hl.dsp.exec_cmd(noctCall .. "notification-clear-active"))
...
-- Focus on monitors (moved off the number row so SUPER + number is free for workspaces)
...
hl.bind(mainMod .. " + SHIFT + F1", hl.dsp.window.move({ monitor = MONITOR1 }))
hl.bind(mainMod .. " + SHIFT + F2", hl.dsp.window.move({ monitor = MONITOR2 }))
-- hl.bind(mainMod .. " + SHIFT + F3", hl.dsp.window.move({ monitor = MONITOR3 }))

-- Focus on workspace N of the focused monitor
for i = 1, NUM_WPM do
	local key = i % 10
